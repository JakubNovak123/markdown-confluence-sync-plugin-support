 ...
 */
function validatePluginEntry(entry, path) {
    // Entry must be an array
    if (!Array.isArray(entry)) {
        throw new Error(`Plugin entry at ${path} must be an array [plugin, options?]`);
    }

    // Entry must have 1 or 2 elements: [plugin] or [plugin, options]
    if (entry.length === 0 || entry.length > 2) {
        throw new Error(`Plugin entry at ${path} must have 1 or 2 elements [plugin, options?]`);
    }
    ...
    if (typeof plugin !== 'function') {
        throw new Error(`Plugin at ${path}[0] must be a function, got ${typeof plugin}`);
    }

    // Second element (if present) must be an object
    if (options !== undefined) {
        if (typeof options !== 'object' || options === null || Array.isArray(options)) {
            throw new Error(`Plugin options at ${path}[1] must be an object if provided`);
        }
    }
}
